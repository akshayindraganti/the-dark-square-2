...
(() => {
  ...
  function add(slug, size2) {
    const key = `${slug}-${size2}`;
    const found = items.find((i) => i.key === key);
    items = found ? items.map((i) => i.key === key ? { ...i, qty: i.qty + 1 } : i) : [...items, { key, slug, size: size2, qty: 1 }];
    commit();
  }
  ...
  function paintPrice(product) {
    ...
    const add2 = $("[data-pdp-add]");
    if (add2) {
      add2.dataset.addToCart = product.slug;
      add2.dataset.size = String(size);
    }
  ...
  ready(() => {
    ...
    document.title = `${product.name} \xB7 The Dark Square`;
