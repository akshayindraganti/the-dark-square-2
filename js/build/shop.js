...
(() => {
  ...
  function productHref(slug2) {
    return href("product", { slug: slug2 });
  }
  ...
  function findProduct(slug2) {
    return PRODUCTS.find((p) => p.slug === slug2) || PRODUCTS[0];
  }
  ...
  var NO_IMAGE_FILL = "repeating-linear-gradient(135deg, var(--bg3) 0 12px, var(--bg4) 12px 24px)";
  function productTile(product, pos = "center 20%") {
  ...
  function unitPrice(product, size2) {
    return size2 === 50 ? product.p50 : product.p100;
  }
  function add(slug2, size2) {
    const key = `${slug2}-${size2}`;
    const found = items.find((i) => i.key === key);
    items = found ? items.map((i) => i.key === key ? { ...i, qty: i.qty + 1 } : i) : [...items, { key, slug: slug2, size: size2, qty: 1 }];
    commit();
  }
  ...
  function initCart() {
    ...
    document.addEventListener("click", (e) => {
      ...
      if (!trigger) return;
      e.preventDefault();
      const size2 = Number(trigger.dataset.size || 100);
      add(trigger.dataset.addToCart, size2);
      setOpen(true);
    });
    subscribe(paint3);
    paint3(snapshot());
  ...
  function initCoupon() {
    ...
    for (const btn of $$("[data-coupon-close]", modal)) btn.addEventListener("click", close2);
    document.addEventListener("keydown", (e) => {
      if (e.key === "Escape" && !modal.hidden) close2();
    });
  ...
  function paint() {
    ...
    render(
      ...
      el(
        "div",
        { class: "quick__body" },
        el("p", { class: "quick__eyebrow" }, `${p.collection} \xB7 ${p.pct}% cacao`),
  ...
  function openQuickView(nextSlug) {
  ...
  // js/pages/shop.js
  var OCCASIONS = ["All", "Everyday", "Gifting", "Celebration", "Corporate"];
  var COLLECTIONS = ["All", ...COLS.map((c) => c.name)];
  var state = { collection: "All", occasion: "All" };
  function readUrl() {
  ...
  function matches(p) {
    return (state.collection === "All" || p.collection === state.collection) && (state.occasion === "All" || p.occasion === state.occasion);
  }
  function card(p) {
    return el(
      "div",
  ...
  function paint2() {
    ...
    if (title) {
      title.textContent = state.collection === "All" ? "Find Your Square" : state.collection;
    }
    if (blurb && state.collection !== "All") {
      blurb.textContent = (COLS.find((c) => c.name === state.collection) || {}).line;
    }
    const count2 = $("[data-result-count]");
    if (count2) {
      count2.textContent = filtered.length + (filtered.length === 1 ? " chocolate" : " chocolates");
    }
    const grid = $("[data-product-grid]");
    if (grid) {
      render(
        grid,
        filtered.length ? filtered.map(card) : el("p", { class: "empty" }, "Nothing matches those filters yet.")
      );
    }
  ...
  ready(() => {
    ...
    render(
      ...
      COLLECTIONS.map(
        (n) => filterButton("collection", n, n === "All" ? "All chocolates" : n, "filter-row")
      )
    );
    render(
